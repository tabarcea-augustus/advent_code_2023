...
map_input = make_matrix(str_map_input)
length = len(map_input)
...
for _ in range(length):
    visit_matrix.append([0] * width)


# print_matrix(map_input)
# print_matrix(visit_matrix)

def search_left(starting_x, starting_y, map_input, visit_matrix):
    if map_input[starting_x][starting_y].isdigit() and not visit_matrix[starting_x][starting_y]:
        number = []
        visit_matrix[starting_x][starting_y] = 1
        number.append(map_input[starting_x][starting_y])
        current_y = starting_y

        current_y -= 1
        while current_y >= 0 and map_input[starting_x][current_y].isdigit() and not visit_matrix[starting_x][current_y]:
            visit_matrix[starting_x][current_y] = 1
            number.append(map_input[starting_x][current_y])
            current_y -= 1
...
def search_right(starting_x, starting_y, map_input, visit_matrix):
    if map_input[starting_x][starting_y].isdigit() and not visit_matrix[starting_x][starting_y]:
        number = []
        visit_matrix[starting_x][starting_y] = 1
        number.append(map_input[starting_x][starting_y])
        ...
        current_y += 1
        while current_y < width and map_input[starting_x][current_y].isdigit() and not visit_matrix[starting_x][
            current_y]:
            visit_matrix[starting_x][current_y] = 1
            number.append(map_input[starting_x][current_y])
            current_y += 1
...
def search_row(x, y, map_input, visit_matrix):
    numbers = []
    starting_x = x
    starting_y = y
    middle_number = None
    if map_input[starting_x][starting_y].isdigit() and not visit_matrix[starting_x][starting_y]:
        visit_matrix[starting_x][starting_y] = 1
        middle_number = map_input[starting_x][starting_y]

    starting_x = x
    starting_y = y - 1
    number_left = search_left(starting_x, starting_y, map_input, visit_matrix)

    starting_x = x
    starting_y = y + 1
    number_right = search_right(starting_x, starting_y, map_input, visit_matrix)

    if number_left and middle_number and number_right:
        numbers.append(int(str(number_left) + str(middle_number) + str(number_right)))

    if middle_number:
        if number_left:
            numbers.append(int(str(number_left) + str(middle_number)))
        elif number_right:
            numbers.append(int(str(middle_number) + str(number_right)))
        else:
            numbers.append(int(middle_number))
    else:
        if number_left:
            numbers.append(int(number_left))
        if number_right:
            numbers.append(int(number_right))

    return numbers


def search_numbers(x, y, visit_matrix, map_input, length, width):
    numbers = []

    starting_x = x
    starting_y = y - 1
    number = search_left(starting_x, starting_y, map_input, visit_matrix)
    if number:
        numbers.append(number)

    starting_x = x
    starting_y = y + 1
    number = search_right(starting_x, starting_y, map_input, visit_matrix)
    if number:
        numbers.append(number)

    numbers.extend(search_row(x - 1, y, map_input, visit_matrix))
    numbers.extend(search_row(x + 1, y, map_input, visit_matrix))
...
for x in range(length):
    for y in range(width):
        if map_input[x][y] in symbols:
            numbers = search_numbers(x, y, visit_matrix, map_input, length, width)
            final_res.extend(numbers)
            # print_matrix(visit_matrix)
